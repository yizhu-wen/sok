...
class Inferencer(object):
    ...
    def write_wav_to_file(self, wav_data, output_path):
        soundfile.write(output_path, wav_data, self.args.sample_rate, subtype="PCM_16")
        return

    def inference_from_path(self):
        src_mel, _ = get_spectrograms(self.args.source)
        tar_mel, _ = get_spectrograms(self.args.target)
        src_mel = torch.from_numpy(self.normalize(src_mel)).to(self.device)
        tar_mel = torch.from_numpy(self.normalize(tar_mel)).to(self.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', '-m', help='model path', default="vctk_model.ckpt")
