...
class Solver():
	...
	def make_records(self):
		...
		self.log_dir = os.path.join(self.config['out_dir'],"log")
		self.convt_mel_dir = os.path.join(self.config['out_dir'],"infer","mel")
		self.convt_voice_dir = os.path.join(self.config['out_dir'],"infer","voice")
	...
	def get_test_data_loaders(self):
		...
		test_filelist = []
		for filename in self.config["test_filelist"]:
			test_filelist.append([os.path.join(self.config["source_path"], filename),
						 		  os.path.join(self.config["target_path"], filename)])
		self.config["len_test_data"] = len(test_filelist)
	...
	def infer(self):
		...
		mel_npy_file_list=[]
		with torch.no_grad():
			for idx, (spk_emb,input_mel, word) in tqdm.tqdm(enumerate(test_data_loader), desc="Generating mel-spectrograms", total=self.config["len_test_data"]):
